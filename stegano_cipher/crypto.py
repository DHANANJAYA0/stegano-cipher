...
def encrypt_bytes(password: str, plaintext: bytes) -> bytes:
    """Encrypt bytes using AES-256-GCM with PBKDF2-HMAC-SHA256, then add ECC.

    Output format: RS encoded (salt(16) || nonce(12) || ciphertext+tag)
    """
    salt = os.urandom(16)
    ...
    blob = salt + nonce + ct
    rsc = RSCodec(32)
    return bytes(rsc.encode(blob))


def decrypt_bytes(password: str, blob: bytes) -> bytes:
    """Decrypt bytes produced by encrypt_bytes.

    Expect input format: RS encoded (salt(16) || nonce(12) || ciphertext+tag)
    """
    rsc = RSCodec(32)
    try:
        blob = bytes(rsc.decode(blob)[0])
    except Exception as e:
        raise ValueError("ECC decoding failed: " + str(e))
